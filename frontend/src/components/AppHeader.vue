...
<template>
  <header class="header">
    ...
    <Dialog v-model:visible="showUid" modal header="Đổi UID người dùng" :style="{ width: '24rem' }">
      <p class="muted dialog-hint">
        UID dùng để phân biệt người dùng (giai đoạn này chưa cần đăng nhập). Đổi UID sẽ xem như một người dùng khác.
      </p>
      <InputText v-model="draft" class="w-full" aria-label="UID" autofocus @keyup.enter="saveUid" />
      <template #footer>
